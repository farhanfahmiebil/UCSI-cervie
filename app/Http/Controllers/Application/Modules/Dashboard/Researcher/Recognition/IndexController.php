...
//Get Controller Path
namespace App\Http\Controllers\Application\Modules\Dashboard\Researcher\Recognition;
...
class IndexController extends Controller{
  ...
  //Path Header
	...
	**************************************************************************************/
	public function routePath(){
		...
		//Set Route View
		$this->route['view'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.view').'.recognition.';
    $this->route['name'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.name').'.recognition.';
  ...
  /**************************************************************************************
    ...
    return redirect()->route($hyperlink['page']['list'])
                     ...
                     ->with('message','Recognition Added');
...
// dd($data['main']['cervie']['researcher']['recognition']);
  ...
  /**************************************************************************************
    ...
    //If Form Token Exist
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.trim(Auth::id()).'/document/recognition/'.$request->id.'/';
        ...
        //Delete Evidence
        $data['main']['verification'] = $model['cervie']['researcher']['recognition']->needVerification(
          [
            'column'=>[
              'recognition_id'=>$request->id,
              'employee_id'=>Auth::id(),
  ...
  /**************************************************************************************
    ...
    if(!$request->has('form_token')){abort(555,'Form Token Missing');}
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.trim(Auth::id()).'/document/recognition/'.$data['evidence']->table_id.'/';
...
// dd(    $data['evidence']);
    ...
    //Set Asset
    $asset['document'] = '/public/resources/researcher/'.trim(Auth::id()).'/document/recognition/'.$request->id.'/';

    //Set Document
    $hyperlink['document'] = $request->root().'/public/storage/resources/researcher/'.trim(Auth::id()).'/document/recognition/'.$request->id.'/';
...
// dd($data['main']->title);
  ...
  /**************************************************************************************
    ...
    return redirect()->route($hyperlink['page']['view'],['id'=>$request->id])
                     ->with('alert_type','success')
                     ->with('message','Recognition Saved');
