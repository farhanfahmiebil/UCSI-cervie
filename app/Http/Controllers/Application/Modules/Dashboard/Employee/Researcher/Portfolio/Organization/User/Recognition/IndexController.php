...
//Get Controller Path
namespace App\Http\Controllers\Application\Modules\Dashboard\Employee\Researcher\Portfolio\Organization\User\Recognition;
...
use App\Models\UCSI_V2_Education\MSSQL\View\CervieResearcherRecognition AS CervieResearcherRecognitionView;
...
use App\Models\UCSI_V2_Education\MSSQL\Procedure\CervieResearcherRecognition AS CervieResearcherRecognitionProcedure;
...
class IndexController extends Controller{
  ...
  protected $user = 'employee';
    ...
    'category'=>'Researcher Recognition',
		'module'=>'Organization',
		'module_sub'=>'User',
    'item'=>'Recognition',
		'gate'=>''
	...
	**************************************************************************************/
	public function routePath(){
		...
		//Set Route Name
		$this->route['name'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.name').'.researcher.portfolio.organization.user.view.recognition.';

    //Set Route View
		$this->route['view'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.view').'.researcher.portfolio.organization.user.view.recognition.';
    ...
    //General Information - Researcher Recognition
  ...
  **************************************************************************************/
  public function new(Request $request){
    ...
    $data['navigation']['category']['sub'] = $model['navigation']['category']['sub']->getList(
      [
        'column'=>[
          'category'=>'PORTAL',
          'user_type'=>strtoupper('administrator'),
          'navigation_category_code'=>'RECOGNITION',
          'domain_url'=>$request->root()
        ]
      ]
    );
    ...
    //Set Table Researcher Recognition
    $data['table']['column']['cervie']['researcher']['recognition'] = $this->getDataTable(
      [
        'category'=>'recognition'
      ]
    );

    //Set Main Data Researcher Recognition
    $data['main']['cervie']['researcher']['recognition'] = $this->getData(
      [
        'eloquent'=>'pagination',
        'category'=>'recognition',
        'column'=>[
    ...
    $data['cervie']['researcher']['table']['control'] = $model['cervie']['researcher']['table']['control']->readRecord(
      [
        'column'=>[
          'table_control_id'=>'cervie_researcher_recognition'
        ]
      ]
    );
  ...
  **************************************************************************************/
  public function create(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'create':
        ...
        $this->getValidateData($request);
          ...
          //Create Main
          $result['main']['create'] = $model['cervie']['researcher']['recognition']->createRecord(
            [
              'column'=>[
                'employee_id'=>Auth::id(),
        ...
        if($request->has('document')){
          ...
          foreach($request->file('document') as $key=>$value){
            ...
            //Set Path Folder
            $path['folder'] = 'public/resources/researcher/'.trim(Auth::id()).'/document/recognition/'.$result['main']['create']->last_insert_id.'/';;
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ...
                     ->with('message','Researcher Recognition Added');
  ...
  **************************************************************************************/
  public function list(Request $request){
    ...
    //Set Model Award
    $model['cervie']['researcher']['recognition'] = new CervieResearcherRecognitionView();

    //Get General Award Type
    foreach($data['general']['representation']['category'] as $key=>$value){

      //Set Main Data Researcher Publication
      $data['main']['cervie']['researcher']['recognition'][$value->representation_category_id] = $model['cervie']['researcher']['recognition']->getList(
        [
      ...
      $data['table']['column']['cervie']['researcher']['recognition'][$value->representation_category_id] = $this->getDataTable(
        [
  ...
  public function delete(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'delete':
        ...
        //Delete Main
        $result['main']['delete'] = $model['cervie']['researcher']['recognition']->deleteRecord(
          [
            'column'=>[
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/recognition/'.$request->id.'/';
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ->with('alert_type','success')
                     ->with('message','Research Recognition Deleted');
  ...
  /**************************************************************************************
    ...
    if(!$request->has('form_token')){abort(555,'Form Token Missing');}
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/recognition/'.$data['evidence']->table_id.'/';
  ...
  **************************************************************************************/
  public function view(Request $request){
    ...
    //Set Main
    $data['main'] = $model['cervie']['researcher']['recognition']->readRecord(
      [
        'column'=>[
          'employee_id'=>Auth::id(),
          'recognition_id'=>$request->id
    ...
    //Set Model
     ...
     if($data['main']->need_verification){
       ...
       //Get Employee Profile
       $data['cervie']['researcher']['log']['recognition'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'table_name'=>'cervie_researcher_recognition',
       ...
       $data['cervie']['researcher']['log']['evidence'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'main_table_name'=>'cervie_researcher_recognition',
    ...
    //Set Asset
    $asset['document'] = '/public/resources/researcher/'.$request->employee_id.'/document/recognition/'.$request->id.'/';

    //Set Document
    $hyperlink['document'] = $request->root().'/storage/resources/researcher/'.$request->employee_id.'/document/recognition/'.$request->id.'/';
  ...
  /**************************************************************************************
    ...
    //If Form Token Exist
      ...
      //Create Main
      $result['main']['update'] = $model['cervie']['researcher']['recognition']->updateRecord(
        [
          'column'=>[
            'recognition_id'=>$request->id,
            'employee_id'=>Auth::id(),
    ...
    return redirect()->route($hyperlink['page']['view'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id,'id'=>$request->id])
                     ->with('alert_type','success')
                     ->with('message','Researcher Recognition Saved');
  ...
  **************************************************************************************/
  public function getData($data){
    ...
    //Set Data
    $data = $model['cervie']['researcher']['recognition']->getList(
      [
