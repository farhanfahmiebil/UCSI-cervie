...
//Get Controller Path
namespace App\Http\Controllers\Application\Modules\Dashboard\Employee\Researcher\Portfolio\Organization\User\Intellectual\Property\Licensing;
...
class IndexController extends Controller{
  ...
  protected $user = 'employee';
	...
	**************************************************************************************/
	public function routePath(){
		...
		//Set Route Name
		$this->route['name'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.name').'.researcher.portfolio.organization.user.view.intellectual.property.licensing.';

    //Set Route View
		$this->route['view'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.view').'.researcher.portfolio.organization.user.view.intellectual.property.licensing.';
    ...
    //General Information - Researcher Licensing
  ...
  **************************************************************************************/
  public function create(Request $request){
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ...
                     ->with('message','Researcher Licensing Added');
  ...
  public function delete(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/licensing/'.$request->id.'/';
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ->with('alert_type','success')
                     ->with('message','Research Award Deleted');
  ...
  /**************************************************************************************
    ...
    if(!$request->has('form_token')){abort(555,'Form Token Missing');}
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/licensing/'.$data['evidence']->table_id.'/';
  ...
  **************************************************************************************/
  public function view(Request $request){
    ...
    //Set Model
     ...
     if($data['main']->need_verification){
       ...
       //Get Employee Profile
       $data['cervie']['researcher']['log']['licensing'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'table_name'=>'cervie_researcher_licensing',
       ...
       $data['cervie']['researcher']['log']['evidence'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'main_table_name'=>'cervie_researcher_licensing',
    ...
    //Set Asset
    $asset['document'] = '/public/resources/researcher/'.$request->employee_id.'/document/licensing/'.$request->id.'/';

    //Set Document
    $hyperlink['document'] = $request->root().'/storage/resources/researcher/'.$request->employee_id.'/document/licensing/'.$request->id.'/';
  ...
  /**************************************************************************************
    ...
    return redirect()->route($hyperlink['page']['view'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id,'id'=>$request->id])
                     ->with('alert_type','success')
                     ->with('message','Researcher Licensing Saved');
  ...
  **************************************************************************************/
  public function getDataTable($data){
    ...
    $table = [
        ...
        2 => [
            'icon' => '<i class="bi bi-file-earmark-text"></i>', // Using Bootstrap icon for Licensing Title
            'name' => 'Licensing Title',
        ],
        3 => [
            'icon' => '<i class="bi bi-calendar"></i>', // Using Bootstrap icon for Date Start
            'name' => 'Date Start',
        ],
        4 => [
            'icon' => '<i class="bi bi-calendar"></i>', // Using the same icon for Date End
            'name' => 'Date End',
        ],
        5 => [
