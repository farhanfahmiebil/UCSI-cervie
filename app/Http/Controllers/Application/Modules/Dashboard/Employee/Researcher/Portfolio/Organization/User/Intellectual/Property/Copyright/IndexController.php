...
//Get Controller Path
namespace App\Http\Controllers\Application\Modules\Dashboard\Employee\Researcher\Portfolio\Organization\User\Intellectual\Property\Copyright;
...
use App\Models\UCSI_V2_Education\MSSQL\Procedure\CervieResearcherCopyright AS CervieResearcherCopyrightProcedure;
...
class IndexController extends Controller{
  ...
  protected $user = 'employee';
	...
	**************************************************************************************/
	public function routePath(){
		...
		//Set Route Name
		$this->route['name'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.name').'.researcher.portfolio.organization.user.view.intellectual.property.copyright.';

    //Set Route View
		$this->route['view'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.view').'.researcher.portfolio.organization.user.view.intellectual.property.copyright.';
    ...
    //General Information - Researcher Copyright
  ...
  **************************************************************************************/
  public function new(Request $request){
    ...
    $data['cervie']['researcher']['table']['control'] = $model['cervie']['researcher']['table']['control']->readRecord(
      [
        'column'=>[
          'table_control_id'=>'cervie_researcher_copyright'
        ]
      ]
    );
    ...
    $data['general']['status'] = $model['general']['status'] ->selectBox(
      [
        'column'=>[
          'table'=>'cervie_researcher_copyright'
        ]
      ]
    );
  ...
  **************************************************************************************/
  public function create(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'create':
        ...
        $this->getValidateData($request);
          ...
          //Set Model
          $model['cervie']['researcher']['copyright'] = new CervieResearcherCopyrightProcedure();

          //Create Main
          $result['main']['create'] = $model['cervie']['researcher']['copyright']->createRecord(
            [
              'column'=>[
                'employee_id'=>Auth::id(),
        ...
        if($request->has('document')){
          ...
          foreach($request->file('document') as $key=>$value){
            ...
            //Set Path Folder
            $path['folder'] = 'public/resources/researcher/'.trim(Auth::id()).'/document/copyright/'.$result['main']['create']->last_insert_id.'/';;
            ...
            $result['evidence']['create'] = $model['cervie']['researcher']['evidence']->createRecord(
              [
                'column'=>[
                  ...
                  'table_name'=>'cervie_researcher_copyright',
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ...
                     ->with('message','Researcher Copyright Added');
  ...
  public function delete(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'delete':
        ...
        //Delete Main
        $result['main']['delete'] = $model['cervie']['researcher']['copyright']->deleteRecord(
          [
            'column'=>[
              'copyright_id'=>$request->id,
              'employee_id'=>$request->employee_id,
            ]
          ]
        );
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/copyright/'.$request->id.'/';
  ...
  /**************************************************************************************
    ...
    if(!$request->has('form_token')){abort(555,'Form Token Missing');}
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/copyright/'.$data['evidence']->table_id.'/';
        ...
        // $data['main']['verification'] = $model['cervie']['researcher']['copyright']->needVerification(
        //   [
        //     'column'=>[
        //       'copyright_id'=>$data['evidence']->table_id,
        //       'employee_id'=>$request->employee_id,
        //       'updated_by'=>Auth::id()
        //     ]
        //   ]
  ...
  **************************************************************************************/
  public function view(Request $request){
    ...
    //Set Main
    $data['main'] = $model['cervie']['researcher']['copyright']->readRecord(
      [
        'column'=>[
          'employee_id'=>Auth::id(),
          'copyright_id'=>$request->id
    ...
    //Set Model
     ...
     if($data['main']->need_verification){
       ...
       //Get Employee Profile
       $data['cervie']['researcher']['log']['copyright'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'table_name'=>'cervie_researcher_copyright',
       ...
       $data['cervie']['researcher']['log']['evidence'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'main_table_name'=>'cervie_researcher_copyright',
    ...
    //Set Asset
    $asset['document'] = '/public/resources/researcher/'.$request->employee_id.'/document/copyright/'.$request->id.'/';

    //Set Document
    $hyperlink['document'] = $request->root().'/storage/resources/researcher/'.$request->employee_id.'/document/copyright/'.$request->id.'/';
  ...
  /**************************************************************************************
    ...
    //If Form Token Exist
      ...
      //Create Main
      $result['main']['update'] = $model['cervie']['researcher']['copyright']->updateRecord(
        [
          'column'=>[
            'employee_id'=>Auth::id(),
            'copyright_id'=>$request->id,
    ...
    return redirect()->route($hyperlink['page']['view'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id,'id'=>$request->id])
                     ->with('alert_type','success')
                     ->with('message','Researcher Copyright Saved');
