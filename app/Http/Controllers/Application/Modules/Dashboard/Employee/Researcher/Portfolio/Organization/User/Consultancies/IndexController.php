...
//Get Controller Path
namespace App\Http\Controllers\Application\Modules\Dashboard\Employee\Researcher\Portfolio\Organization\User\Consultancies;
...
class IndexController extends Controller{
  ...
  protected $user = 'employee';
    ...
    'category'=>'Researcher Consultancies',
		'module'=>'Organization',
		'module_sub'=>'User',
    'item'=>'Consultancies',
		'gate'=>''
	...
	**************************************************************************************/
	public function routePath(){
		...
		//Set Route Name
		$this->route['name'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.name').'.researcher.portfolio.organization.user.view.consultancies.';

    //Set Route View
		$this->route['view'] = config('routing.'.$this->application.'.modules.dashboard.'.$this->user.'.view').'.researcher.portfolio.organization.user.view.consultancies.';
    ...
    //General Information - Researcher consultancies
  ...
  **************************************************************************************/
  public function new(Request $request){
    ...
    $data['navigation']['category']['sub'] = $model['navigation']['category']['sub']->getList(
      [
        'column'=>[
          'category'=>'PORTAL',
          'user_type'=>strtoupper('administrator'),
          'navigation_category_code'=>'consultancies',
          'domain_url'=>$request->root()
        ]
      ]
    );
    ...
    /*  Researcher consultancies
   	**************************************************************************************/

    //Set Table Researcher consultancies
    ...
    //Set Main Data Researcher consultancies
    $data['main']['cervie']['researcher']['consultancies'] = $this->getData(
      [
        'eloquent'=>'pagination',
        'category'=>'consultancies',
        'column'=>[
  ...
  **************************************************************************************/
  public function create(Request $request){
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ...
                     ->with('message','Researcher Consultancies Added');
...
// dd($this->encrypter->decrypt($request->form_token));
  ...
  public function delete(Request $request){
    ...
    //If Form Token Exist
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/consultancies/'.$request->id.'/';
    ...
    return redirect()->route($hyperlink['page']['list'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id])
                     ->with('alert_type','success')
                     ->with('message','Research consultancies Deleted');
  ...
  /**************************************************************************************
    ...
    if(!$request->has('form_token')){abort(555,'Form Token Missing');}
      ...
      case 'delete':
        ...
        //Set Path Folder
        $path['folder'] = 'public/resources/researcher/'.$request->employee_id.'/document/consultancies/'.$data['evidence']->table_id.'/';
  ...
  **************************************************************************************/
  public function view(Request $request){
    ...
    //Set Model
     ...
     if($data['main']->need_verification){
       ...
       //Get Employee Profile
       $data['cervie']['researcher']['log']['consultancies'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'table_name'=>'cervie_researcher_consultancies',
       ...
       // dd($data['cervie']['researcher']['log']['consultancies']['qualification'] );
       ...
       $data['cervie']['researcher']['log']['evidence'] = $model['cervie']['researcher']['log']->readRecord(
         [
           'column'=>[
             'employee_id'=>$request->employee_id,
             'main_table_name'=>'cervie_researcher_consultancies',
    ...
    //Set Asset
    $asset['document'] = '/public/resources/researcher/'.$request->employee_id.'/document/consultancies/'.$request->id.'/';

    //Set Document
    $hyperlink['document'] = $request->root().'/storage/resources/researcher/'.$request->employee_id.'/document/consultancies/'.$request->id.'/';
  ...
  /**************************************************************************************
    ...
    return redirect()->route($hyperlink['page']['view'],['organization_id'=>$request->organization_id,'employee_id'=>$request->employee_id,'id'=>$request->id])
                     ->with('alert_type','success')
                     ->with('message','Researcher consultancies Saved');
...
// dd($data);
    ...
    //Set Data
    $data = $model['cervie']['researcher']['consultancies']->getList(
      [
