<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    @if(!in_array(Request::segment(7),array('checkout')))
      //Refresh Cart Total
      refreshCartTotal();
...
// console.log(value.item_description+'-'+value.item_is_taxable+'-'+value.item_tax_rate+'-'+value.item_tax);
  ...
  console.log(data.value.category_id);
      var image = data.value.item_image ?
    'data:image/jpeg;base64,' + data.value.item_image :
    (['1', '12'].includes(data.value.category_id) ?
        '{{ asset('images/default/no_image_food.png') }}' :
        (['2', '13'].includes(data.value.category_id) ?
            '{{ asset('images/default/no_image_drink.png') }}' :
            '{{ asset('images/default/no_image.png') }}'));
        ...
        case 'cart':
          ...
          if(Object.keys(data.value).length != 0){
            ...
            html += '<div class="card px-1 py-3 card-body flex-row border-0" data-order-body-id="'+data.value.order_body_id+'">';
              ...
              html += '<hr class="p-0 m-0 border-secondary">';
              html += '<div>';
                html += '<span>Total : </span>';
                html += '<span>RM'+data.value.item_total+'</span>';
              html += '</div>';
              ...
              html += '<div>';
                ...
                html += '<i class="bi bi-trash text-danger"></i>'
                html += '</a>';
        ...
        case 'checkout':
          ...
          if(Object.keys(data.value).length != 0){
            ...
            html += '<div class="card px-1 py-3 card-body flex-row border border-top-1" data-order-body-id="'+data.value.order_body_id+'">';
              ...
              html += '<div class="flex-grow-1">';
              html += '<h6 class="mb-0">'+data.value.item_description+'</h6>';
              html += '<span class="d-inline-flex align-items-center my-1">Qty: <input type="text" class="form-control ms-1 update-item-quantity quantity" maxlength="2" name="quantity" onkeypress="return inputSetting(\'number\',event)" value="'+data.value.item_quantity+'"></span>';
              ...
              html += '<textarea class="form-control lock update-item-remark" name="remark" rows="3" cols="80">'+((data.value.item_order_remark != null)?data.value.item_order_remark:'')+'</textarea>';
        ...
        case 'submit':
          ...
          if(Object.keys(data.value).length != 0){
            ...
            html += '<div class="card px-1 py-3 card-body flex-row border border-top-1" data-order-body-id="'+data.value.order_body_id+'">';
              ...
              html += '<textarea class="form-control lock update-item-remark" name="remark" rows="3" cols="80" readonly>'+((data.value.item_order_remark != null)?data.value.item_order_remark:'')+'</textarea>';
              // html += '<textarea class="form-control lock update-item-remark" name="remark" rows="3" cols="80">'+data.value.item_order_remark+'</textarea>';
              html += '<div class="d-flex fw-bold fs-6">';
              // html += '<span class="d-flex align-self-center">';
              html += 'Menu Delivered to Customer:';
              // html += '</span>';
              html += '<i class="h4 bi '+((parseInt(data.value.item_fulfill))?'bi-check text-success':'bi-x text-danger')+'"></i>';
              html += '</div>';
              html += '</div>';
