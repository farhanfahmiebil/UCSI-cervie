...
<div class="card">
  ...
  <div class="card-header">
    ...
    <div class="d-flex align-items-center justify-content-between">
      ...
      <!-- title -->
      <h4 class="card-title mb-2">Researcher Consultancies</h4>
      <!-- end title -->
    ...
    <div class="table-responsive">
      ...
      <table class="table table-striped">
        ...
        <thead class="bg-danger text-white mx-3">
          ...
          @if(isset($data['table']['column']['cervie']['researcher']['consultancies']) && count($data['table']['column']['cervie']['researcher']['consultancies']) >= 1)

              {{-- Get Table Column Data --}}
              @foreach($data['table']['column']['cervie']['researcher']['consultancies'] as $key => $value)
        ...
        <tbody>
          ...
          @if(count($data['main']['cervie']['researcher']['consultancies']) > 0)

            {{-- Get Researcher Position Data --}}
            @foreach($data['main']['cervie']['researcher']['consultancies'] as $key=>$value)

              <tr id="{{ $value->consultancies_id }}" class="bg-danger">

                {{-- Check if Checkbox Status True --}}
                @if($checkbox['status'])
                  <td>
                    <div class="form-check">
                      <input type="checkbox" name="id[]" class="form-check-input selectBox" value="{{ $value->consultancies_id }}"/>
                    </div>
                  </td>
                @endif
                ...
                <td>
                  <a href="{{ route($hyperlink['page']['view'],['organization_id'=>request()->route('organization_id'),'employee_id'=>request()->route('employee_id'),'id'=>$value->consultancies_id]) }}" class="btn btn-sm btn-secondary">
                    <i class="bi bi-pencil"></i>
                  </a>

                  <a data-href="{{ route($hyperlink['page']['delete']['main'],['organization_id'=>request()->route('organization_id'),'employee_id'=>request()->route('employee_id'),'id'=>$value->consultancies_id]) }}" class="btn-delete-main btn btn-sm btn-danger">
          ...
          @else
            ...
            <tr>
              <td colspan="{{ count($data['table']['column']['cervie']['researcher']['consultancies']) }}">No Data</td>
            </tr>
      ...
      <div class="col-12 pt-3">
        ...
        @if(count($data['main']['cervie']['researcher']['consultancies']) >= 1)

          <!-- paginate -->
          {{ $data['main']['cervie']['researcher']['consultancies']->appends(request()->input())->links(Config::get('routing.application.modules.dashboard.researcher.layout').'.navigation.pagination.index',['navigation'=>['alignment'=>'center']]) }}
          <!-- end paginate -->
