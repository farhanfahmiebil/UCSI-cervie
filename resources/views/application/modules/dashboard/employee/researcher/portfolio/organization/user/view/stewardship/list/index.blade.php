...
<div class="card">
  ...
  <div class="card-header">
    ...
    <div class="d-flex align-items-center justify-content-between">
      ...
      <!-- title -->
      <h4 class="card-title mb-2">Researcher Stewardship</h4>
      <!-- end title -->
  ...
  <div class="card-body">
    ...
    @if(count($data['general']['representation']['category']) > 0)
      ...
      @foreach($data['general']['representation']['category'] as $key=>$value)
        ...
        {{-- Check Researcher Stewardship Representation Category Exist --}}
        @if(count($data['main']['cervie']['researcher']['stewardship'][$value->representation_category_id]) > 0)
          ...
          {{-- Get Researcher Stewardship Data --}}
          @foreach($data['main']['cervie']['researcher']['stewardship'][$value->representation_category_id] as $k=>$v)

            {{-- Check General Researcher Representation Category Matched With Researcher Stewardship Representation Category --}}
            @if($value->representation_category_id == $v->representation_category_id)
              ...
              <div class="table-responsive">
                ...
                <table class="table table-striped">
                  ...
                  <thead class="bg-danger text-white mx-3">
                    ...
                    @if(isset($data['table']['column']['cervie']['researcher']['stewardship'][$value->representation_category_id]) && count($data['table']['column']['cervie']['researcher']['stewardship'][$value->representation_category_id]) >= 1)

                      {{-- Get Table Column Data --}}
                      @foreach($data['table']['column']['cervie']['researcher']['stewardship'][$value->representation_category_id] as $k=>$v)
                  ...
                  <tbody>
                    ...
                    {{-- Check Researcher Stewardship Exist --}}
                    @if(count($data['main']['cervie']['researcher']['stewardship'][$value->representation_category_id]) > 0)
                      ...
                      {{-- If Representation Category Matched --}}
                        @if($value->representation_category_id == $v->representation_category_id)
                          ...
                          @php
                            $counter = 0
                          @endphp
                          <tr id="{{ $v->stewardship_id }}">

                            {{-- Check if Checkbox Status True --}}
                            @if($checkbox['status'])
                              <td>
                                <div class="form-check-label">
                                  <input type="checkbox" name="id[]" class="form-check-input selectBox" value="{{ $v->stewardship_id }}"/>
                                </div>
                              </td>
                            @endif
                            ...
                            <td>
                              <a href="{{ route($hyperlink['page']['view'],['organization_id'=>request()->route('organization_id'),'employee_id'=>request()->route('employee_id'),'id'=>$v->stewardship_id]) }}" class="btn btn-sm btn-secondary">
                                <i class="bi bi-pencil"></i>
                              </a>

                              <a data-href="{{ route($hyperlink['page']['delete']['main'],['organization_id'=>request()->route('organization_id'),'employee_id'=>request()->route('employee_id'),'id'=>$v->stewardship_id]) }}" class="btn-delete-main btn btn-sm btn-danger">
                      ...
                      @endforeach
                      {{-- End Get Researcher Stewardship Data --}}

                    @else

                      <tr class="text-center">
                        <td colspan="{{ count($data['table']['column']['cervie']['researcher']['stewardship'][$v->representation_category_id]) }}">No Data</td>
                      </tr>

                    @endif
                    {{-- End Check Researcher Stewardship Exist --}}
              ...
              @break
            @endif
            {{-- End Check General Representation Category Matched With Researcher Stewardship Representation Category --}}
