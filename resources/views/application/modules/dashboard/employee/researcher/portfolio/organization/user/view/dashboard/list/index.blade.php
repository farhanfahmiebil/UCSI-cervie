...
<div class="card">
  ...
  <div class="card-header">
    ...
    <div class="d-flex align-items-center justify-content-between">
      ...
      <!-- title -->
      <h4 class="card-title mb-2">Researcher Verification</h4>
      <!-- end title -->
    ...
    <div class="table-responsive">
      ...
      <table class="table table-striped">
        ...
        <thead class="bg-danger text-white mx-3">
          ...
          @if(isset($data['table']['column']['cervie']['researcher']['log']) && count($data['table']['column']['cervie']['researcher']['log']) >= 1)

              {{-- Get Table Column Data --}}
              @foreach($data['table']['column']['cervie']['researcher']['log'] as $key => $value)
        ...
        <tbody>
          ...
          @if(count($data['main']['cervie']['researcher']['log']) > 0)

            {{-- Get Researcher Position Data --}}
            @foreach($data['main']['cervie']['researcher']['log'] as $key=>$value)

              <tr id="{{ $value->id }}" class="bg-danger">

                {{-- Check if Checkbox Status True --}}
                @if($checkbox['status'])
                  <td>
                    <div class="form-check">
                      <input type="checkbox" name="id[]" class="form-check-input selectBox" value="{{ $value->id }}"/>
                    </div>
                  </td>
                @endif
                ...
                <td>{{ $value->full_name . ' (' . $value->created_by . ')' }}</td>
                <td>{{ $value->created_at }}</td>
                <td><span class="badge shade-{{ (($value->need_verification)?'yellow':'green') }}">{{ (($value->need_verification)?'Pending':'Verified') }}</span></td>
                <td>
                  <a href="{{ route($hyperlink['page']['view'] . $value->route . '.view',['organization_id'=>request()->route('organization_id'),'employee_id'=>request()->route('employee_id'),'id'=>$value->id]) }}" class="btn btn-sm btn-secondary">
          ...
          @else
            ...
            <tr>
              <td colspan="{{ count($data['table']['column']['cervie']['researcher']['log']) }}">No Data</td>
            </tr>
      ...
      <div class="col-12 pt-3">
        ...
        @if(count($data['main']['cervie']['researcher']['log']) >= 1)

          <!-- paginate -->
          {{ $data['main']['cervie']['researcher']['log']->appends(request()->input())->links(Config::get('routing.application.modules.dashboard.researcher.layout').'.navigation.pagination.index',['navigation'=>['alignment'=>'center']]) }}
          <!-- end paginate -->
