            ...
            <table class="table align-middle">
              ...
              <thead>
                <tr class="text-start">
                  ...
                  {{-- Check Table Column Main Exist --}}
                  @if(count($data['table']['column']['main']) >= 1)

                    {{-- Get Table Column Main --}}
                    @foreach($data['table']['column']['main'] as $key=>$value)

                      <td class="{{ ((isset($value['class'])?$value['class']:'')) }}" >{!! ((isset($value['icon'])?$value['icon']:'')) !!} {{ ((isset($value['name'])?$value['name']:'')) }}</td>

                    @endforeach
                    {{-- End Get Table Column Main --}}

                  @else
                    <th>Column Not Defined</th>
                  @endif

                  {{-- End Check Table Column Main Exist --}}
