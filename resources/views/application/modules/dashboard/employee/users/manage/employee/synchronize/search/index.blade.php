...
<!-- form -->
<form action="{{route($hyperlink['page']['synchronize']['process'])}}" method="POST">
{{csrf_field()}}
  ...
  <div class="card-header">
    <h3>
      ...
      <br>
      Status : {!! (count($data['main'])?'<span class="text-success">Found<span>':'<span class="text-danger">Not Found<span>') !!}
    </h3>
  </div>
  <!-- end card header -->
  ...
  <div class="d-flex gap-2 justify-content-end">
    ...
    <input type="hidden" name="synchronize_category" value="{{ request()->synchronize_category }}">
    <input type="hidden" name="form_token" value="{{ $form_token['synchronize'] }}">
    <a href="{{ route($hyperlink['page']['list']) }}" class="btn btn-dark">Back to List</a>

    @if(!$synchronize['status'])
      <button type="submit" class="btn btn-primary">Process to Synchronize</button>
    @endif
